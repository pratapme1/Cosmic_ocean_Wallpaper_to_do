package com.cosmicocean.effects

import kotlin.math.floor
import kotlin.math.sqrt

/**
 * Simplex Noise implementation for procedural generation
 * Based on Stefan Gustavson's implementation
 */
object NoiseLibrary {

    // Simplex noise permutation table
    private val perm = intArrayOf(
        151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225,
        140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23, 190, 6, 148,
        247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203, 117, 35, 11, 32,
        57, 177, 33, 88, 237, 149, 56, 87, 174, 20, 125, 136, 171, 168, 68, 175,
        74, 165, 71, 134, 139, 48, 27, 166, 77, 146, 158, 231, 83, 111, 229, 122,
        60, 211, 133, 230, 220, 105, 92, 41, 55, 46, 245, 40, 244, 102, 143, 54,
        65, 25, 63, 161, 1, 216, 80, 73, 209, 76, 132, 187, 208, 89, 18, 169,
        200, 196, 135, 130, 116, 188, 159, 86, 164, 100, 109, 198, 173, 186, 3, 64,
        52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118, 126, 255, 82, 85, 212,
        207, 206, 59, 227, 47, 16, 58, 17, 182, 189, 28, 42, 223, 183, 170, 213,
        119, 248, 152, 2, 44, 154, 163, 70, 221, 153, 101, 155, 167, 43, 172, 9,
        129, 22, 39, 253, 19, 98, 108, 110, 79, 113, 224, 232, 178, 185, 112, 104,
        218, 246, 97, 228, 251, 34, 242, 193, 238, 210, 144, 12, 191, 179, 162, 241,
        81, 51, 145, 235, 249, 14, 239, 107, 49, 192, 214, 31, 181, 199, 106, 157,
        184, 84, 204, 176, 115, 121, 50, 45, 127, 4, 150, 254, 138, 236, 205, 93,
        222, 114, 67, 29, 24, 72, 243, 141, 128, 195, 78, 66, 215, 61, 156, 180
    )

    // Doubled permutation table for wrapping
    private val permMod12 = IntArray(512) { i ->
        perm[i and 255] % 12
    }

    // Gradient vectors for 2D
    private val grad2 = arrayOf(
        floatArrayOf(1f, 1f), floatArrayOf(-1f, 1f), floatArrayOf(1f, -1f), floatArrayOf(-1f, -1f),
        floatArrayOf(1f, 0f), floatArrayOf(-1f, 0f), floatArrayOf(1f, 0f), floatArrayOf(-1f, 0f),
        floatArrayOf(0f, 1f), floatArrayOf(0f, -1f), floatArrayOf(0f, 1f), floatArrayOf(0f, -1f)
    )

    /**
     * 2D Simplex Noise
     * @param xin X coordinate
     * @param yin Y coordinate
     * @return Noise value in range [-1, 1]
     */
    fun simplexNoise2D(xin: Float, yin: Float): Float {
        val F2 = 0.5f * (sqrt(3.0f) - 1.0f)
        val s = (xin + yin) * F2
        val i = floor(xin + s).toInt()
        val j = floor(yin + s).toInt()

        val G2 = (3.0f - sqrt(3.0f)) / 6.0f
        val t = (i + j) * G2
        val X0 = i - t
        val Y0 = j - t
        val x0 = xin - X0
        val y0 = yin - Y0

        val i1: Int
        val j1: Int
        if (x0 > y0) {
            i1 = 1
            j1 = 0
        } else {
            i1 = 0
            j1 = 1
        }

        val x1 = x0 - i1 + G2
        val y1 = y0 - j1 + G2
        val x2 = x0 - 1.0f + 2.0f * G2
        val y2 = y0 - 1.0f + 2.0f * G2

        val ii = i and 255
        val jj = j and 255
        val gi0 = permMod12[ii + perm[jj and 255]]
        val gi1 = permMod12[ii + i1 + perm[(jj + j1) and 255]]
        val gi2 = permMod12[ii + 1 + perm[(jj + 1) and 255]]

        var n0 = 0f
        var n1 = 0f
        var n2 = 0f

        var t0 = 0.5f - x0 * x0 - y0 * y0
        if (t0 >= 0) {
            t0 *= t0
            n0 = t0 * t0 * dot(grad2[gi0], x0, y0)
        }

        var t1 = 0.5f - x1 * x1 - y1 * y1
        if (t1 >= 0) {
            t1 *= t1
            n1 = t1 * t1 * dot(grad2[gi1], x1, y1)
        }

        var t2 = 0.5f - x2 * x2 - y2 * y2
        if (t2 >= 0) {
            t2 *= t2
            n2 = t2 * t2 * dot(grad2[gi2], x2, y2)
        }

        return 70.0f * (n0 + n1 + n2)
    }

    private fun dot(g: FloatArray, x: Float, y: Float): Float {
        return g[0] * x + g[1] * y
    }

    /**
     * Fractal Brownian Motion (FBM) - layers of noise for complexity
     * @param x X coordinate
     * @param y Y coordinate
     * @param octaves Number of noise layers (default 5)
     * @param persistence How much each octave contributes (default 0.5)
     * @param lacunarity Frequency multiplier between octaves (default 2.0)
     * @return Noise value in range [-1, 1]
     */
    fun fbm(x: Float, y: Float, octaves: Int = 5, persistence: Float = 0.5f, lacunarity: Float = 2.0f): Float {
        var total = 0f
        var frequency = 1f
        var amplitude = 1f
        var maxValue = 0f

        for (i in 0 until octaves) {
            total += simplexNoise2D(x * frequency, y * frequency) * amplitude

            maxValue += amplitude

            amplitude *= persistence
            frequency *= lacunarity
        }

        return total / maxValue
    }

    /**
     * Domain warping - distorts the noise coordinates for more organic patterns
     * @param x X coordinate
     * @param y Y coordinate
     * @param warpStrength How much to warp (default 0.5)
     * @return Warped noise value
     */
    fun domainWarp(x: Float, y: Float, warpStrength: Float = 0.5f): Float {
        val warpX = simplexNoise2D(x * 3f, y * 3f)
        val warpY = simplexNoise2D(x * 3f + 100f, y * 3f + 100f)

        val warpedX = x + warpX * warpStrength
        val warpedY = y + warpY * warpStrength

        return fbm(warpedX, warpedY)
    }
}
